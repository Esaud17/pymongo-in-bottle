...
def post_comment_like():
    ...
    try:
        ...
        if "num_likes" in comment:
           ...
           print "Incrementing the num likes counter to ", num_likes
        else:
           print "Set num likes to %d" % num_likes
           
        num_likes_key = 'comments.%d.num_likes' % ordinal
        last_error = posts.update({'permalink':permalink},
        { '$set': {num_likes_key:num_likes} })
    except:
        print "Could not update the collection, error"
        print "Unexpected error:", sys.exc_info()[0]
